...
import styles from './catalogSzt.module.css'
import { Icon } from '../../../../UI/icons/icon'
import { InputUI } from '../../../../UI/input/input'
import { closeFilterPanel, closeMessage, showMessage, toggleExtraPanel, toggleStatusGroupKsg } from '../../../../optionsSlice'
import { applyUpdateDataset, markingForDeletion, setCurrentGroup, setDatasetSzt, setFilterDataset } from './datasetSztSlice'
...
export const CatalogSzt = () => {
  ...
  const availableUpdating = useSelector((state) => state.datasetSzt.dataset).filter((ksg) => ksg.status === 'update' || ksg.status === 'new' || (ksg.status === 'remove' && ksg))
  const availableChoice = useSelector((state) => state.datasetSzt.dataset).filter((ksg) => ksg.choice && ksg)
  ...
  useEffect(() => {
    ...
    loaderData(URL.URL_GET_DATA_MODEL, data)
      .then((result) => {
        ...
        dispatch(setDatasetSzt({ dataset: result.dataset, groups: result.dataset2, variants: result.dataset3, quotients: result.dataset4 }))
      })
