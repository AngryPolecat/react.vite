...
/*
dataset - основной набор данных
dataset2 -
*/

const initialState = { mainDataset: [], optionalDataset: [], currentGroup: null, filterDataset: '' }
...
const datasetModelSlice = createSlice({
  ...
  reducers: {
    setDatasetModel(state, action) {
      state.mainDataset = action.payload.dataset.map((ksg) => ({ ...ksg, choice: false, status: null, showMenu: false }))
      state.optionalDataset = action.payload.dataset2.map((grp) => ({ ...grp, status: null }))
