...
import { setCurrentDepartment, setListDepartments } from './listTarifSlice'
...
export const ListDepartment = () => {
  const departments = useSelector((state) => state.listTarif.departments)
    ...
    if (currentLpuId && currentModel) {
      loaderData(URL.URL_GET_LIST_DEP, data)
        .then((result) => {
          ...
          dispatch(setListDepartments(result.dataset))
        })
